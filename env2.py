...
class Robot_Gridworld(tk.Tk, object):
    ...
    def build_gridworld(self):
        ...
        bomb2_center = origin + np.array([UNIT * 3, UNIT*6])
        ...
        food_center = origin + np.array([UNIT * 4, UNIT * 7])
        self.food = self.gridworld.create_rectangle(
            food_center[0] - 15, food_center[1] - 15,
            food_center[0] + 15, food_center[1] + 15,
            fill='orange')
    ...
    def step(self, action):
        ...
        if next_state == self.gridworld.coords(self.treasure):
            ...
            reward = -10
            terminal = True
        elif next_state == self.gridworld.coords(self.food):
            reward = 10
            terminal = True
        else:
            reward = -0.1
